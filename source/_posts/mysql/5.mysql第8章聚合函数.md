...
#1.where子句可否使用组函数（聚合函数）进行过滤?
# 答：不能，where 后面过滤条件不能跟聚合函数， 而having 后面可以跟聚合函数

#2.查询公司员工 工资的 最大值,最小值,平均值,总和
select max(salary),min(salary),avg(salary),sum(salary)
from employees;

#3.查询各job_id的员工 工资的 最大值,最小值,平均值,总和
...
# 6.查询各个管理者手下 员工的 最低工资,其中最低工资不能低于6000, 没有管理者的员工不计算在内
/*
...
# 7.查询 所有部门 的名字,location_id,员工数量 和 平均工资, 并按平均工资降序
